...
// Fonction pour désactiver les animations si elles sont trop perturbantes
function toggleAnimations(disable = false) {
...
function bindCommandTabs() {
  tabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      ...
      renderCommandsSafe(tab.dataset.category);
    });
  });
}
...
function bindAnimationToggle() {
  const toggleBtn = document.querySelector("#toggle-animations");
  if (!toggleBtn) return;

  let animationsDisabled = localStorage.getItem('animationsDisabled') === 'true';
  ...
  toggleBtn.addEventListener("click", () => {
    ...
    if (animationsDisabled) {
      ...
      toggleBtn.textContent = 'Animations';
...
function bindHelpTabs() {
  ...
  helpTabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      ...
      tab.classList.add("active");
      tab.setAttribute("aria-selected", "true");
      renderHelpCommandsSafe(tab.dataset.category);
    });
  });
}
...
// Sanitize command rendering
function renderCommandsSafe(category = "music") {
    ...
    .map(
      (command) => `
        <article class="command-item">
          ...
          <span class="command-badge">${escapeHTML(command.label)}</span>
        </article>
      `
    )
    .join("");
...
// Sanitize help commands rendering
function renderHelpCommandsSafe(category = "music") {
  const items = commands[category] || commands.music;
      ...
      (command) => `
        <article class="help-item">
          <div class="help-item-icon">
            <span class="ui-icon">${escapeHTML(getCategoryIcon(category))}</span>
          </div>
          <div class="help-item-content">
            <strong class="help-item-name">${escapeHTML(command.name)}</strong>
            <span class="help-item-label">${escapeHTML(command.label)}</span>
            <p class="help-item-description">${escapeHTML(command.description)}</p>
          </div>
        </article>
      `
    )
...
try {
  ...
  if (tabs.length > 0) bindCommandTabs();
  if (navToggle) bindNavigation();
  if (inviteLinks.length > 0) bindInviteLinks();
  bindRevealAnimation();
  renderHelpCommandsSafe();
  bindHelpTabs();
  bindAnimationToggle();
  startManaField();
} catch (error) {
